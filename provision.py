...
def new_state(env, xray):
    ...
    state = {"PANEL_PORT": env.get("PANEL_PORT", str(20000 + secrets.randbelow(40000))),
             ...
             "AWG_MTU": env.get("AWG_MTU", "1280" if env.get("AWG_PROTOCOL", "3.1") == "3.1" else "1376"),
    ...
    state["AWG_PROTOCOL"] = env.get("AWG_PROTOCOL", "3.1")
    if awg_profile.version(state) == "3.1":
        state["AWG_PARAMETERS"] = json.dumps(awg_profile.new_profile(), sort_keys=True)
...
def amnezia_key(state, client, config, server_public):
    common = dict(awg_profile.parameters(state, OBFS), I2="", I3="", I4="", I5="")
    ...
    last = dict(common, allowed_ips=["0.0.0.0/0", "::/0"], clientId=client["public"],
                ...
                persistent_keep_alive=awg_profile.keepalive(state), port=int(state["AWG_PORT"]), psk_key=client["psk"], server_pub_key=server_public)
    awg = dict(common, last_config=json.dumps(last), port=state["AWG_PORT"], protocol_version=awg_profile.version(state),
...
def render(bundle, root=ROOT, awg_conf=AWG_CONF, qr=True):
    ...
    validate_state(state)
    root = Path(root)
    obfs = "".join(f"{key} = {value}\n" for key, value in awg_profile.parameters(state, OBFS).items())
    ...
    for client in bundle["clients"]:
        ...
        config = (f"[Interface]\nAddress = {client['ip']}/32\nDNS = 1.1.1.1, 8.8.8.8\n"
                  ...
                  f"AllowedIPs = 0.0.0.0/0, ::/0\nEndpoint = {state['PANEL_HOST']}:{state['AWG_PORT']}\nPersistentKeepalive = {awg_profile.keepalive(state)}\n")
        name = client["name"]
